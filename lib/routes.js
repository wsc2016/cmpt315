...
router.post('/profile', stormpath.loginRequired, function(req, res){
  ...
  db.query(
    "UPDATE users SET city=?, genre=? WHERE id=?", [req.body.city, req.body.genre, req.user.email], function(err, rows){
      if (err) {
          res.json({
              statusCode: 500,
...
router.post('/callouts/delete/:id', stormpath.getUser, function(req, res){
