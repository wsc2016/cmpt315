...
exports.retrieve_conversations = function(db, req, res) {
    db.query(
        "SELECT * FROM users WHERE id IN (SELECT userID FROM conversants WHERE conversationID IN (SELECT DISTINCT conversationID FROM conversants WHERE userID=?) AND userID!=?);", [req.params.userID,req.params.userID], function(err, fields) {
