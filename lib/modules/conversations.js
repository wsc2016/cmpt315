...
exports.create_conversation = function(db, req, res) {
    db.query(
        "INSERT INTO conversations (timeInitiated) VALUES (NOW());",
        function (err) {
            if (err) {
                res.json({
                    statusCode: 500,
                ...
                db.query(
                    ...
                    function (err, results) {
                        if (err) {
                            ...
                            var realID = results[Object.keys(results)[0]];
                            console.log(typeof realID);
                            console.log(realID[Object.keys(realID)[0]]);
                            //var realIDJSON = (JSON.parse(JSON.stringify(realID[0])));
                            var final_conversationID = (realID[Object.keys(realID)[0]]);
                            db.query(
                                ...
                                function (err) {
                                    if (err) {
                                        ...
                                        db.query(`INSERT INTO conversants VALUES (?,'${final_conversationID}');`,
                                            [req.body.email],
                                            function(err){
                                                if (err) {
                                                    res.redirect('back');
